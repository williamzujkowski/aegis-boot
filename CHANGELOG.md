...
- **`direct_install` module foundation** (PR1 of [#274](https://github.com/williamzujkowski/aegis-boot/issues/274)) — first slice of the direct-install flash epic that will eventually replace the `mkusb.sh + dd` pipeline. New `crates/aegis-cli/src/direct_install.rs` ships Rust-native helpers: `partition_stick()` (sgdisk zap + fresh GPT + ESP 400 MB + AEGIS_ISOS rest-of-disk), `format_esp()` (mkfs.fat FAT32 labeled AEGIS_ESP), `format_data_partition()` (mkfs.exfat labeled AEGIS_ISOS). Constants (`ESP_SIZE_MB`, `AEGIS_ISOS_LABEL`, `ESP_TYPE_CODE`, `DATA_TYPE_CODE`) are drift-tested against `scripts/mkusb.sh`'s defaults so a surprise change in either source surfaces as a test failure. 10 new unit tests cover the pure `build_partition_argv` argv-builder (sgdisk shape, both -n flags, embedded size format `+400M`, 2:0:0 default-span, type codes ef00/0700, "EFI System" / AEGIS_ISOS partition names, device path last, both SCSI + NVMe paths). No caller wired up — the flash command still goes through mkusb.sh + dd; `#[allow(dead_code)]` rides at module level until PR2 of #274 adds the `--direct-install` flag and dispatches to these helpers. Backed by nexus-agents consensus vote (2026-04-19, 80% approve, `higher_order` strategy) with security + architect concerns incorporated into the phased plan.

### Bug fixes

- **Close auto-expand automount race** (closes [#272](https://github.com/williamzujkowski/aegis-boot/issues/272)) — on Linux desktops running udisks2 / gvfs (GNOME / KDE / XFCE), `flash --image <smaller-img>` would run sgdisk to expand partition 2, then fail the subsequent `mkfs.exfat` step with `Device or resource busy` because the desktop auto-mounted the freshly-resized partition. Net effect: partition table was correctly expanded but the filesystem was never reformatted — stick usable, but AEGIS_ISOS stuck at the mkusb.sh default size (~1.6 GiB of a 30 GiB partition). Fix: after `partprobe` and before `mkfs.exfat`, run `udevadm settle` to wait for the auto-mount daemon, then use `findmnt -n <dev>` + lazy `umount -l` to release any mount the desktop grabbed. No-op on initramfs / CI (no udev or gvfs running). Also improves the soft-fail warning to accurately describe what failed (just the mkfs, not the expand) and include a manual recovery recipe (`sudo umount /dev/<stick>2 && sudo mkfs.exfat -L AEGIS_ISOS /dev/<stick>2`). Real-hardware validated: 30 GB Cruzer, 2 GB image → `✓ AEGIS_ISOS now spans the full stick` + sda2 blkid confirms exfat.

### Security hardening

- **Refuse group/world-writable `AEGIS_TRUSTED_KEYS` entries** (CWE-732 defense-in-depth) — `iso-probe::minisign::load_trusted_keys` now refuses to load a `.pub` file (and skips an entire directory) when the inode's mode bits include `0o022` (group-write or world-write). Prevents an architectural weakness if the env var ever points at a multi-user shared location: an attacker with write access could otherwise drop a malicious `.pub` file and redirect the trust anchor. Currently safe in the single-user initramfs that's the primary deployment target, but forecloses the foot-gun on operator hosts. New `is_path_safely_owned()` helper is Unix-gated; returns `true` on non-Unix (Windows) where the mode-bit model doesn't map. Failure to stat the path (ENOENT / EACCES) fails closed — returns `false` and logs the refusal via `tracing::warn`. Was flagged in the v0.6.0 security review (2026-04-14) as architecturally weak. 6 new unit tests cover owner-only modes (0o600, 0o644, 0o755), group-writable rejection (0o664), world-writable rejection (0o646, 0o666), missing-file fail-closed, and the end-to-end `load_key_into` skip.

### Release integrity

- **`aegis-boot fetch-image` auto-resolves to the latest release** (PR3 of [#235](https://github.com/williamzujkowski/aegis-boot/issues/235)) — completes the "most automatic" directive. Operators now run `aegis-boot fetch-image` with zero arguments and get the latest release image, cosign-verified against aegis-boot's release-workflow identity (from #267). New `--version TAG` flag pins to a specific release (`aegis-boot fetch-image --version v0.14.0`); `--url URL` still works for arbitrary sources. When both are supplied, `--url` wins with a warning (explicit overrides inferred). Default URL is GitHub's `latest` alias — the redirect to the actual tag doesn't affect cosign verification because the hardcoded identity regex (#267) accepts any tag ref on `release.yml`. New `release_download_url_for_tag()` helper refuses shell-meta / path-traversal in the tag argument (`[A-Za-z0-9._-]{1,32}` only) — defense-in-depth since the tag flows directly into the URL path. 7 new unit tests: default URL shape (anchor + latest + .img), tag-pinned URL builder, prerelease-tag shape (`v1.0.0-rc1`), shell-meta rejection, tag validator accept patterns, `--version` argv parsing (both `--version=...` and `--version ...` forms), dangling `--version` flag rejection. Compose-with-flash one-liner simplifies to `img=$(aegis-boot fetch-image) && aegis-boot flash --image "$img" /dev/sdX` — no URL to look up, no sha256 to pin (cosign is the integrity anchor).

- **`release.yml` builds + signs `aegis-boot.img`** (PR2 of [#235](https://github.com/williamzujkowski/aegis-boot/issues/235)) — extends the release workflow to produce the signed-chain disk image alongside the existing binary artifacts. `mkusb.sh` runs after the CLI build and emits `out/aegis-boot.img` + `out/aegis-boot.img.sha256`; the image is added to the aggregate `SHA256SUMS`, signed by the existing cosign `sign-blob` loop (producing `aegis-boot.img.sig` + `aegis-boot.img.pem` bound to the release workflow's OIDC identity), and uploaded to the GitHub release as four additional assets. Operators can now `aegis-boot fetch-image --url .../aegis-boot.img` and the PR1 auto-verification (shipped in #267) activates — no flag-flipping required. Image contents default to empty `AEGIS_ISOS` (operator uses `aegis-boot fetch` + `add` to populate; keeps the asset small and sidesteps distribution-rights questions on bundling third-party ISOs). CI job timeout bumped 25 → 30 min to accommodate the mkusb.sh run (~3-5 min added). New apt prereqs mirror `mkusb.yml`'s existing list (`shim-signed grub-efi-amd64-signed linux-image-virtual mtools dosfstools exfatprogs gdisk`) — keeps the two image-building environments in sync.
