...
| Windows x86_64 | **Full `flash --direct-install` support** ([#419](https://github.com/aegis-boot/aegis-boot/issues/419) epic closed 2026-04-24). Native `CreateFileW` + `FSCTL_LOCK_VOLUME` + sector-aligned `WriteFile` through `windows-rs`; operator-facing CLI dispatches `diskpart` + `Format-Volume` + raw-stage. Drive enumeration via `Get-Disk`. Builds from source with `cargo install --path crates/aegis-cli`; pre-built binary tracked in [#365](https://github.com/aegis-boot/aegis-boot/issues/365). |
